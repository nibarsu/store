...
if ($_REQUEST["gen_sales_total"] <> ""){
     ...
     $sql="SELECT SID,UID,
     SUM(COST) COST,SUM(AMT) AMT,SUM(PROFIT) PROFIT FROM SO WHERE SO.CTIME BETWEEN '$SDATE' AND '$EDATE'
     AND TYPE IN (2,5) ";
     ...
     $sql.="GROUP BY SID,UID ORDER BY CTIME";
...
if ($_REQUEST["gen_sales_sum"] <> ""){
     ...
     $sql="SELECT CTIME,SID,UID,UNPAY,CASH,CARD,LAYAPAY,FCURR,FCASH,PREPAY,REMARK,SO.CNAME,
     SUM(COST) COST,SUM(AMT) AMT,SUM(PROFIT) PROFIT FROM SO WHERE SO.CTIME BETWEEN '$SDATE' AND '$EDATE'
     AND TYPE IN (2,5) ";
     ...
     $sql.="GROUP BY CTIME,SID,UID,UNPAY,CASH,CARD,LAYAPAY,FCURR,FCASH,PREPAY,REMARK ORDER BY CTIME";
...
if ($_REQUEST["gen_sales_dtl"] <> ""){
     ...
     $sql="SELECT SO.ID,SID,UID,COST,AMT,PROFIT,UNPAY,CASH,CARD,LAYAPAY,FCURR,FCASH,PREPAY,REMARK,SO.CTIME,SO.CNAME
     FROM SO WHERE SO.CTIME BETWEEN '$SDATE' AND '$EDATE' AND TYPE IN (2,5) ";
     ...
     while($row=$rows->FetchRow()){
            ...
            $sql="SELECT PT.NAME PROD_TYPE,PM.NAME PROD_NAME,PD.CODE,PD.COST FROM SO_D D,PROD_M PM,PROD_D PD,PROD_TYPE PT WHERE D.ID=".$row["ID"]." 
            AND D.PID=PM.ID 
            AND D.PID=PD.ID 
