...
if ($_REQUEST["gen_inv_list2"] <> ""){
     ...
     //          AND I.QTY<>0
     //          ";
     $sql="SELECT S.NAME STORE_NAME,PD.NAME PROD_TYPE,M.NAME PROD_NAME,SUM(D.TYPE) QTY,IF (D.TYPE=1,'在庫','轉倉') IT_NAME
              FROM PROD_M M,
                   PROD_D D,
                   PROD_TYPE PD,
                   STORE_M S
     ...
     $sql.="  GROUP BY S.NAME,PD.NAME,M.NAME,D.TYPE ORDER BY S.NAME,PD.NAME,M.NAME,D.TYPE";
     //print $sql;
     $rows=$conn->Execute($sql);
     while($row=$rows->FetchRow()){
